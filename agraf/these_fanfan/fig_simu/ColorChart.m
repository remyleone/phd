%% Latex xcolor Color chart
GreenYellow = [0.85 1 0.31];
Yellow = [1 1 0];
Goldenrod = [1 0.9 0.16];
Dandelion = [1 0.71 0.16];
Apricot = [1 0.68 0.48];
Peach = [1 0.5 0.3];
Melon = [1 0.54 0.5];
YellowOrange = [1 0.58 0];
Orange = [1 0.39 0.13];
BurntOrange = [1 0.49 0];
Bittersweet = [0.76 0.19 0];
RedOrange = [1 0.23 0.13];
Mahogany = [0.65 0.0975 0.0845];
Maroon = [0.68 0.0884 0.2176];
BrickRed = [0.72 0.0792 0.0432];
Red = [1 0 0];
OrangeRed = [1 0 0.5];
RubineRed = [1 0 0.87];
WildStrawberry = [1 0.04 0.61];
Salmon = [1 0.47 0.62];
CarnationPink = [1 0.37 1];
Magenta = [1 0 1];
VioletRed = [1 0.19 1];
Rhodamine = [1 0.18 1];
Mulberry = [0.6468 0.098 0.98];
RedViolet = [0.6138 0.066 0.66];
Fuchsia = [0.4876 0.0828 0.92];
Lavender = [1 0.52 1];
Thistle = [0.88 0.41 1];
Orchid = [0.68 0.36 1];
DarkOrchid = [0.6 0.2 0.8];
Purple = [0.55 0.14 1];
Plum = [0.5 0 1];
Violet = [0.21 0.12 1];
RoyalPurple = [0.25 0.1 1];
BlueViolet = [0.1344 0.0864 0.96];
Periwinkle = [0.43 0.45 1];
CadetBlue = [0.38 0.43 0.77];
CornflowerBlue = [0.35 0.87 1];
MidnightBlue = [0.0114 0.4959 0.57];
NavyBlue = [0.06 0.46 1];
RoyalBlue = [0 0.5 1];
Blue = [0 0 1];
Cerulean = [0.06 0.89 1];
Cyan = [0 1 1];
ProcessBlue = [0.04 1 1];
SkyBlue = [0.38 1 0.88];
Turquoise = [0.15 1 0.8];
TealBlue = [0.1372 0.98 0.6468];
Aquamarine = [0.18 1 0.7];
BlueGreen = [0.15 1 0.67];
Emerald = [0 1 0.5];
JungleGreen = [0.01 1 0.48];
SeaGreen = [0.31 1 0.5];
Green = [0 1 0];
ForestGreen = [0.0792 0.88 0.1056];
PineGreen = [0.06 0.75 0.3075];
LimeGreen = [0.5 1 0];
YellowGreen = [0.56 1 0.26];
SpringGreen = [0.74 1 0.24];
OliveGreen = [0.216 0.6 0.03];
RawSienna = [0.55 0.154 0];
Sepia = [0.3 0.051 0];
Brown = [0.4 0.076 0];
Tan = [0.86 0.58 0.44];
Gray = [0.5 0.5 0.5];
Black = [0 0 0];
White = [1 1 1];

%% x11 Color chart

AliceBlue = [0.94 0.97 1];
AntiqueWhite = [0.98 0.92 0.84];
Aqua = [0 1 1];
Aquamarine = [0.5 1 0.83];
Azure = [0.94 1 1];
Beige = [0.96 0.96 0.86];
Bisque = [1 0.89 0.77];
Black = [0 0 0];
BlanchedAlmond = [1 0.92 0.8];
Blue = [0 0 1];
BlueViolet = [0.54 0.17 0.89];
Brown = [0.65 0.16 0.16];
Burlywood = [0.87 0.72 0.53];
CadetBlue = [0.37 0.62 0.63];
Chartreuse = [0.5 1 0];
Chocolate = [0.82 0.41 0.12];
Coral = [1 0.5 0.31];
Cornflower = [0.39 0.58 0.93];
Cornsilk = [1 0.97 0.86];
Crimson = [0.86 0.08 0.24];
Cyan = [0 1 1];
DarkBlue = [0 0 0.55];
DarkCyan = [0 0.55 0.55];
DarkGoldenrod = [0.72 0.53 0.04];
DarkGray = [0.66 0.66 0.66];
DarkGreen = [0 0.39 0];
DarkKhaki = [0.74 0.72 0.42];
DarkMagenta = [0.55 0 0.55];
DarkOliveGreen = [0.33 0.42 0.18];
DarkOrange = [1 0.55 0];
DarkOrchid = [0.6 0.2 0.8];
DarkRed = [0.55 0 0];
DarkSalmon = [0.91 0.59 0.48];
DarkSeaGreen = [0.56 0.74 0.56];
DarkSlateBlue = [0.28 0.24 0.55];
DarkSlateGray = [0.18 0.31 0.31];
DarkTurquoise = [0 0.81 0.82];
DarkViolet = [0.58 0 0.83];
DeepPink = [1 0.08 0.58];
DeepSkyBlue = [0 0.75 1];
DimGray = [0.41 0.41 0.41];
DodgerBlue = [0.12 0.56 1];
Firebrick = [0.7 0.13 0.13];
FloralWhite = [1 0.98 0.94];
ForestGreen = [0.13 0.55 0.13];
Fuchsia = [1 0 1];
Gainsboro = [0.86 0.86 0.86];
GhostWhite = [0.97 0.97 1];
Gold = [1 0.84 0];
Goldenrod = [0.85 0.65 0.13];
Gray = [0.75 0.75 0.75];
WebGray = [0.5 0.5 0.5];
Green = [0 1 0];
WebGreen = [0 0.5 0];
GreenYellow = [0.68 1 0.18];
Honeydew = [0.94 1 0.94];
HotPink = [1 0.41 0.71];
IndianRed = [0.8 0.36 0.36];
Indigo = [0.29 0 0.51];
Ivory = [1 1 0.94];
Khaki = [0.94 0.9 0.55];
Lavender = [0.9 0.9 0.98];
LavenderBlush = [1 0.94 0.96];
LawnGreen = [0.49 0.99 0];
LemonChiffon = [1 0.98 0.8];
LightBlue = [0.68 0.85 0.9];
LightCoral = [0.94 0.5 0.5];
LightCyan = [0.88 1 1];
LightGoldenrod = [0.98 0.98 0.82];
LightGray = [0.83 0.83 0.83];
LightGreen = [0.56 0.93 0.56];
LightPink = [1 0.71 0.76];
LightSalmon = [1 0.63 0.48];
LightSeaGreen = [0.13 0.7 0.67];
LightSkyBlue = [0.53 0.81 0.98];
LightSlateGray = [0.47 0.53 0.6];
LightSteelBlue = [0.69 0.77 0.87];
LightYellow = [1 1 0.88];
Lime = [0 1 0];
LimeGreen = [0.2 0.8 0.2];
Linen = [0.98 0.94 0.9];
Magenta = [1 0 1];
Maroon = [0.69 0.19 0.38];
WebMaroon = [0.5 0 0];
MediumAquamarine = [0.4 0.8 0.67];
MediumBlue = [0 0 0.8];
MediumOrchid = [0.73 0.33 0.83];
MediumPurple = [0.58 0.44 0.86];
MediumSeaGreen = [0.24 0.7 0.44];
MediumSlateBlue = [0.48 0.41 0.93];
MediumSpringGreen = [0 0.98 0.6];
MediumTurquoise = [0.28 0.82 0.8];
MediumVioletRed = [0.78 0.08 0.52];
MidnightBlue = [0.1 0.1 0.44];
MintCream = [0.96 1 0.98];
MistyRose = [1 0.89 0.88];
Moccasin = [1 0.89 0.71];
NavajoWhite = [1 0.87 0.68];
NavyBlue = [0 0 0.5];
OldLace = [0.99 0.96 0.9];
Olive = [0.5 0.5 0];
OliveDrab = [0.42 0.56 0.14];
Orange = [1 0.65 0];
OrangeRed = [1 0.27 0];
Orchid = [0.85 0.44 0.84];
PaleGoldenrod = [0.93 0.91 0.67];
PaleGreen = [0.6 0.98 0.6];
PaleTurquoise = [0.69 0.93 0.93];
PaleVioletRed = [0.86 0.44 0.58];
PapayaWhip = [1 0.94 0.84];
PeachPuff = [1 0.85 0.73];
Peru = [0.8 0.52 0.25];
Pink = [1 0.75 0.8];
Plum = [0.87 0.63 0.87];
PowderBlue = [0.69 0.88 0.9];
Purple = [0.63 0.13 0.94];
WebPurple = [0.5 0 0.5];
RebeccaPurple = [0.4 0.2 0.6];
Red = [1 0 0];
RosyBrown = [0.74 0.56 0.56];
RoyalBlue = [0.25 0.41 0.88];
SaddleBrown = [0.55 0.27 0.07];
Salmon = [0.98 0.5 0.45];
SandyBrown = [0.96 0.64 0.38];
SeaGreen = [0.18 0.55 0.34];
Seashell = [1 0.96 0.93];
Sienna = [0.63 0.32 0.18];
Silver = [0.75 0.75 0.75];
SkyBlue = [0.53 0.81 0.92];
SlateBlue = [0.42 0.35 0.8];
SlateGray = [0.44 0.5 0.56];
Snow = [1 0.98 0.98];
SpringGreen = [0 1 0.5];
SteelBlue = [0.27 0.51 0.71];
Tan = [0.82 0.71 0.55];
Teal = [0 0.5 0.5];
Thistle = [0.85 0.75 0.85];
Tomato = [1 0.39 0.28];
Turquoise = [0.25 0.88 0.82];
Violet = [0.93 0.51 0.93];
Wheat = [0.96 0.87 0.7];
White = [1 1 1];
WhiteSmoke = [0.96 0.96 0.96];
Yellow = [1 1 0];
YellowGreen = [0.6 0.8 0.2];

%% Misc
BurlyWood = [222 184 135]./255;
FireBrick = [178 34 34]./255;


