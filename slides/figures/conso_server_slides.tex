...
\begin{document}
  ...
  \begin{tikzpicture}[shorten >=1pt, node distance=2cm, on grid, auto]
   \node[draw, circle] (R) {$S$};
   \node[draw, circle] (right) [right=5cm of R] {$D$};
  ...
  \path[->]
    ...
    (R)  edge [bend right=70, thick,olive]   node  [below, black]   {(1) $\req$}   (right)
    (right)  edge [bend left, dashed, thick,olive]   node  [black]   {(2) $\ack_r$}   (R)
    (right)  edge [bend right=70, thick,cyan]   node  [above, black]   {(3) $\ans$}   (R)
    (R)  edge [bend left, dashed, thick,cyan]   node  [above, black]   {(4) $\ack_a$}   (right)
  ;
  \end{tikzpicture}
