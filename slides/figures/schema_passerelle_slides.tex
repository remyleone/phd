...
\begin{document}
  ...
  % les nœuds
  \node[] (gw) {\includegraphics[scale=.2]{router.png}};
  \node[above=.5cm of gw] (spec_gw) {\begin{tabular}{c}Interop\\(Passerelle)\end{tabular}};
...
\path
  ...
  % Réseau contraint
  (gw) edge[<->, very thick, Sepia] node [above] {\begin{tabular}{c}Protocoles\\adaptés\end{tabular}} node [below] {(6LoWPAN)} (1)
  ...
  (gw.east) edge[<->, very thick, ForestGreen] node [above] {\begin{tabular}{c}Protocoles\\usuels\end{tabular}} node [below] {(IP)} (cloud)
