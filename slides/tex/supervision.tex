...
\begin{frame}{État de l'art}
  \begin{block}{Supervision active}
    \begin{itemize}
      \item Envoi de statut périodique de la part des noeuds (``health report'', SNMP)
      \item Sondes tierces
    \end{itemize}
  \end{block}
  ...
  \pnote{
    Par contre le protocole SNMP peut etre lourd à implémenter (support des MIB Management Information Base pas forcément exhaustif, plusieurs versions du protocole (3 version), dans la troisième qui est sécurisée: pas de négociation de chiffrement =>Doit connaitre à l'avance le chiffreur utilisé)
  }
  \pnote{
...
\begin{frame}{Modélisation des transmission}
  \begin{figure}
    \input{figures/impact_transmission}
  \end{figure}
  \begin{block}{Estimation ``étoilée''}
    \begin{itemize}
      \item Source et destination
    \end{itemize}
  \end{block}
  \begin{block}{Estimation ``maillée''}
    \begin{itemize}
      \item Route complète
    \end{itemize}
  \end{block}
...
\begin{frame}{Validation expérimentale}
    ...
    \caption{$\delta$ final par profondeur}
    \end{figure}  
...
\begin{frame}{Recalibration périodique}
